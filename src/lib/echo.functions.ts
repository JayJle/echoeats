...
export const searchRestaurants = createServerFn({ method: "POST" })
  ...
  .handler(async function* ({ data }): AsyncGenerator<SearchStreamChunk, void, unknown> {
    ...
    const langDirective = isEn
      ? `\n## OUTPUT LANGUAGE (MANDATORY)\nAll human-readable fields (aiSummary, pros[].text, cons[].text, matchDetails[].label, hardFilterChecks[].note) MUST be in **English only**. No CJK characters. If a source review is Chinese/Japanese, paraphrase into English; do NOT include the original. Keep placeId and enum values exactly as specified.\n`
      : `\n## 输出语言（强制）\n所有人类可读字段（aiSummary、pros[].text、cons[].text、matchDetails[].label、hardFilterChecks[].note）**必须用简体中文**。禁止整句英文/日文，禁止"原文+括号翻译"。若评论是英/日文，转写为中文；专有名词（店名、地铁站名）可保留原文。placeId 和 enum 值原样保留。\n`;
