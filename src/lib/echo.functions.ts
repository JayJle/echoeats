...
- **Tabelog 仅作辅助参考，不参与 matchScore 打分（重要）**：matchScore 的依据**只能**是 Google 数据（rating / userRatingCount / priceLevel / editorialSummary / openNow / 营业状态）+ realWorldReviews（reviewHighlights / commonComplaints）+ 用户硬条件/偏好的核对结果。candidate.tabelog 仅在以下两种受限场景使用：(a) Google 信号薄弱（无评分或 userRatingCount < 30）时，Tabelog 评分可作为信心补充，**最多影响 ±2 分**，不得用 Tabelog 单独抬高或压低 matchTier；(b) aiSummary 末尾可用「Tabelog 上 X.XX」一句话做交叉验证文案。**当 Tabelog 与 Google 评分/价格/口碑冲突时，一律以 Google + realWorldReviews 为准**，不要因 Tabelog 偏低就扣分、也不要因 Tabelog 高就加分超过 ±2。**禁止把 Tabelog 写进 matchDetails**（避免误导用户以为 Tabelog 参与匹配）。pros / cons 仍只能引用 realWorldReviews，不得用 tabelog.summary 充当「网友评价」。tabelog 为 null 时完全不影响判断。
